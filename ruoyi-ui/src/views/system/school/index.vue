<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="schoolList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="名字" align="center" prop="schoolName" v-if="true"/>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="名字" prop="schoolName">
          <el-input v-model="form.schoolName" placeholder="请输入学校名字" />
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        level: [
          { required: true, message: "高校级别不能为空", trigger: "blur" }
        ],
        department: [
          { required: true, message: "高校所属不能为空", trigger: "blur" }
        ],
        other: [
          { required: false, message: "其他不能为空", trigger: "blur" }
        ]
      }
    };
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除高校管理编号为"' + ids + '"的数据项？').then(() => {
        this.loading = true;
